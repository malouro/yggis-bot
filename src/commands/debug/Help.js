...
export default class Help extends Command {
	...
	buildCommandHelpMenu({ commandPrefix }, command, invalidUsage = false) {
		const { usage, name, description } = command;
		const { chainableArgs, nonChainableArgs } = this.getCommandArgs(command);

		/* Get all appropriate text to display */
		/* <CommandName> Command */
		const title =
			(typeof this.t('COMMANDS', this.translationKey, 'commandHeader') ===
				'function' &&
				this.t('COMMANDS', this.translationKey, 'commandHeader')(name)) ||
			`**${name} Command**`;
		...
		/* Was the Help command used incorrectly? Let the user know */
		const invalidUsageText =
			(typeof this.t('COMMANDS', this.translationKey, 'invalidCommandUsage') ===
				'function' &&
				this.t(
					'COMMANDS',
					this.translationKey,
		...
		const missingDescriptionText =
			this.t('COMMANDS', this.translationKey, 'missingArgumentDescription') ||
			'(Missing description.)';
		...
		return [
			...
			commandUsage,
			'',
			this.getDetailedArgsDescription(usage.args, name),
		].join('\n');
	}
	...
	buildCategoryHelpMenu({ commandPrefix }, category) {
		...
		return [title, descriptionText, commandList].join('\n');
	}
