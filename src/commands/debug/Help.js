...
export default class Help extends Command {
	...
	buildCommandHelpMenu({ commandPrefix }, command, invalidUsage = false) {
		const { usage, name, translationKey: tKeyFromCommand } = command;
		const { chainableArgs, nonChainableArgs } = this.getCommandArgs(command);

		const translationKey = tKeyFromCommand || name.toLocaleLowerCase();
		const translate = key => this.t('COMMANDS', translationKey, key);
